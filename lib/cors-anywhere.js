...
var customHttpHandler = require('./custom-http-handler').customHttpHandler;
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (location.host === 'track') {
      var handlerResult = customHttpHandler(req, res);
    ...
    var parseJiraServer = function(link) {
      try {
        console.log(link);
        return link
          .replace('https://', '')
    ...
    Object.keys(corsAnywhere.setHeaders).forEach(function(header) {
      var headerValue = corsAnywhere.setHeaders[header];

      switch (header) {
        case 'origin':
        case 'referer':
          headerValue = parseJiraServer(req.url) ?
            headerValue.replace('any', parseJiraServer(req.url)) :
            corsAnywhere.setHeaders[header];
          break;
      }
